...
function vowelCount(str) {
  ...
  lowerCaseStr.forEach((char) => {
    // for each vowel element, if char exists in vowels array
    if (vowels.indexOf(char) !== -1) {
      // if it doesn't exist, add it to object and set its value to 1
      if (!vowelObj[char]) {
        vowelObj[char] = 1
      } else {
        //if it does exist as a key, increment its value by 1
        vowelObj[char] += 1
      }
    }
  })
...
function filterByValue(arr, key) {
  return (objsWithKey = arr.filter((obj) => obj.hasOwnProperty(key)))
}
